...
def store_fb_contacts():
    ...
    while row:
        ...
        contacts_dict[row[0]] = (
        extracted_data["profileFbid"], extracted_data["name"]["displayName"], dis_num, univ_num,
        extracted_data["hugePictureUrl"])
        row = contacts_cursor.fetchone()
    ...
    contacts_output_file = OUTPUT + SEP + "contacts.tsv"
...
def store_fb_thread_data():
    if os.path.isfile(threadsdb):
        threads_conn = sqlite3.connect(threadsdb)
    else:
        print("threadsdb does not exist")
        return
    threads_query = "select messages.msg_id, messages.thread_key, messages.text, messages.sender, messages.timestamp_ms" \
                    " from messages, threads where messages.thread_key=threads.thread_key " \
    ...
    while row:
        ...
        messages_dict[row[MESGS_MSG_ID_COL_INDX]] = [
            row[MESGS_THREAD_ID_COL_INDX], textstr, sender, row[MESGS_TIMESTAMP_COL_INDX]]
    ...
    messages_output_file = OUTPUT + SEP + "messages.tsv"
    ...
    for value in sorted_messages_dict:
        ...
        if messages_dict[value][MESGS_TIMESTAMP_INDX] > 0:
            datetimestr = datetime.datetime.fromtimestamp(messages_dict[value][MESGS_TIMESTAMP_INDX] / 1000).strftime(
                '%Y-%m-%dT%H:%M:%S')
        else:
