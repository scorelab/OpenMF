...
from os_check import ADB
try:
	from os_check import SUC
except:
	pass
...
if 'error' in Popen([ADB,'get-state'],stdout=PIPE,stderr=STDOUT).stdout.read().decode('UTF-8'):
    ...
    print(" ADB serial: "+ ADB_SN); REPORT.append(["ADB serial", ADB_SN])


##### Build properties
BUILDPROP = Popen([ADB, 'shell', SUC, 'cat', '/system/build.prop'],stdout=PIPE,stderr=STDOUT).stdout.read().decode('UTF-8')
# Manufacturer & Model
...
try:
	...
	pass
ANDROID_TIME = Popen([ADB, 'shell', SUC, 'date', '+%F %T %Z'],stdout=PIPE,stderr=STDOUT).stdout.read().decode('UTF-8').replace('\r\n', '')
try:
...
if Popen([ADB, 'shell', SUC, 'ls', SIM_LOC],stdout=PIPE,stderr=STDOUT).stdout.read().decode('UTF-8').replace('\r', '').replace('\n', '') == SIM_LOC:
	SIM_DATA = Popen([ADB, 'shell', SUC, 'cat', SIM_LOC],stdout=PIPE,stderr=STDOUT).stdout.read().decode('UTF-8').replace('\r', '')
