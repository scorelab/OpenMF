...
contacts_output_file = "phone_contacts.tsv"
...
messages_output_file = "messages_conversation.tsv"
...
for value in messages_dict:
    ...
    if(messages_dict[value][MESGS_SORT_TIMESTAMP_COL_INDX] > 0):
        datetimestr = datetime.datetime.fromtimestamp(messages_dict[value][MESGS_SORT_TIMESTAMP_COL_INDX]/1000).strftime('%Y-%m-%dT%H:%M:%S')
    else:
...
calllogs_query = "select _id, formatted_number, date, duration, type, name, geocoded_location from calls order by _id ;"
...
calllogs_output_file = "phone_calllogs.tsv"
...
for value in calllogs_dict:
    ...
    if(calllogs_dict[value][CALLLOGS_DATE_COL_INDX] > 0):
        datetimestr = datetime.datetime.fromtimestamp(calllogs_dict[value][CALLLOGS_DATE_COL_INDX]/1000).strftime('%Y-%m-%dT%H:%M:%S')
    else:
