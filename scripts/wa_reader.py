...
def store_wa_contacts():
    ...
    contacts_output_file = OUTPUT + SEP + "wa_contacts.tsv"
...
def store_wa_messages():
    if os.path.isfile(msgstoredb):
        msgstore_conn = sqlite3.connect(msgstoredb)
    else:
        print("msgstoredb does not exist")
        return
    msgstore_query = "select _id, key_remote_jid, key_from_me, data, timestamp, media_url," \
                     " media_wa_type, media_name, remote_resource, received_timestamp " \
                     "from messages order by key_remote_jid, timestamp;"
    ...
    while row:
        ...
        messages_dict[row[0]] = (
            _id, key_remote_jid, key_from_me, data, timestamp, media_url, media_wa_type, media_name, remote_resource,
            received_timestamp)
    ...
    messages_output_file = OUTPUT + SEP + "messages_wa.tsv"
    ...
    for value in messages_dict:
        ...
        if messages_dict[value][MESGS_TIMESTAMP_COL_INDX] > 0:
            datetimestr = datetime.datetime.fromtimestamp(
                messages_dict[value][MESGS_TIMESTAMP_COL_INDX] / 1000).strftime('%Y-%m-%dT%H:%M:%S')
        else:
            datetimestr = str(messages_dict[value][MESGS_TIMESTAMP_COL_INDX])

        if messages_dict[value][MESGS_REC_TIMESTAMP_COL_INDX] > 0:
            rec_datetimestr = datetime.datetime.fromtimestamp(
                messages_dict[value][MESGS_REC_TIMESTAMP_COL_INDX] / 1000).strftime('%Y-%m-%dT%H:%M:%S')
        else:
